...
class Model(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        self.act = get_activation(activation)
        self.attention_block = ConT(dim=last_channel, local_size=32, num_heads=4)
        self.attention_block1 = ConT(dim=last_channel, local_size=16, num_heads=4)
...
class ConT(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns: Output tensor of shape [B, S, D]
        '''
        B, S, D = x.shape
        # 1. Compute Q, K, V
        qkv = self.qkv(x).reshape(B, S, 3, self.num_heads, D // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # [B, num_heads, S, head_dim]

        # 2. Reshape for channel-based clustering
        q_reshaped = q.permute(0, 2, 3, 1).reshape(B, S, D)  # Reshape Q to [B, S, D]
        channel_cluster = q_reshaped.transpose(1, 2)  # Reshape to [B, D, S]
        cmean = channel_cluster.mean(dim=-1, keepdim=True)  # Mean across S
        cstd = channel_cluster.std(dim=-1, keepdim=True)  # Standard deviation across S

        # 3. Binary clustering based on mean
        cthreshold = cmean  # Using mean as the threshold
        binary_mask = (channel_cluster > cthreshold).float()  # Binary mask based on mean

        # 4. Apply clustering and reshape back
        clustered_channels = binary_mask * channel_cluster + (1 - binary_mask) * channel_cluster
        clustered_channels = clustered_channels.transpose(1, 2)  # Back to [B, S, D]

        # 5. Update Q, retain K and V for attention
        q = clustered_channels.reshape(B, self.num_heads, S, D // self.num_heads)  # [B, num_heads, S, head_dim]

        # 6. Attention computation
        attn = (q @ k.transpose(-2, -1)) * self.scale  # Attention scores: [B, num_heads, S, S]
        attn = self.softmax(attn)  # Apply softmax
        attn = self.attn_drop(attn)

        # 7. Weighted sum of values
        x = (attn @ v).transpose(1, 2).reshape(B, S, D)  # [B, S, D]
